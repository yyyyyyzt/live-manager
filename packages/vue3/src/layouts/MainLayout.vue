        ...
        <div v-if="showLogout" class="user-menu-wrapper" ref="userMenuRef">
          <div class="user-menu" @click="toggleUserMenu">
            <div class="user-avatar">
              ...
              />
              <User v-else class="user-icon-fallback" :size="22" />
            </div>
            <span class="user-name">{{ displayUserName }}</span>
            <ChevronDown class="user-chevron" :size="16" :class="{ rotated: userMenuOpen }" />
          </div>
      ...
      <aside class="main-sidebar">
        <div class="sidebar-content">
          ...
          <nav class="sidebar-menu">
            ...
            >
              <Gift class="menu-icon" :size="20" />
              <span>礼物配置</span>
            </div>
            <div class="menu-divider" />
            <div class="menu-item" @click="router.push('/host')">
    ...
    if (urlOverride) {
      try {
        ...
        if (loginRes.code === 0 && loginRes.data?.userId) {
          ...
          localStorage.setItem('sdk_app_id', String(sdkAppId ?? ''));
          sdkUserId.value = userId;
