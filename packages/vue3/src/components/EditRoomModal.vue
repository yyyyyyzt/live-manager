<template>
  <t-dialog
    ...
    @close="handleClose"
  >
    <t-form class="edit-room-form" :label-width="100" :colon="false">
      <!-- 直播间ID -->
      ...
      <!-- 直播间标题 -->
      <t-form-item label="直播间标题" :required-mark="true">
        <t-input
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <t-button theme="primary" shape="round" :loading="saving" :disabled="saving || !formData.title.trim() || coverUploadRef?.isValidating || coverUploadRef?.hasUrlError" @click="handleSubmit">
          {{ saving ? '保存中...' : '保存' }}
        </t-button>
      </div>
    </template>
...
import { ChevronDownIcon, ChevronRightIcon, AddIcon, CloseIcon } from 'tdesign-icons-vue-next';
  ...
  try {
    ...
    for (const info of customInfos.value) {
      ...
      if (keyBytes > CUSTOM_INFO_LIMITS.maxKeyBytes) {
        MessagePlugin.error(`Key "${trimmedKey}" 超过 ${CUSTOM_INFO_LIMITS.maxKeyBytes} 字节限制`);
        saving.value = false;
        return;
      }
      ...
      if (valueBytes > CUSTOM_INFO_LIMITS.maxValueBytes) {
        MessagePlugin.error(`Key "${trimmedKey}" 的 Value 超过 2KB 限制`);
        saving.value = false;
        return;
      }
    ...
    if (totalValueBytes > CUSTOM_INFO_LIMITS.maxTotalValueBytes) {
      MessagePlugin.error(`所有 Value 总大小超过 16KB 限制`);
      saving.value = false;
      return;
    }
    ...
    if (response.ErrorCode === 0 || response.ErrorCode === undefined) {
      MessagePlugin.success('直播间信息已更新');
      emit('success', {
        roomName: formData.value.title.trim(),
        coverUrl: coverUrl,
      });
      ...
      MessagePlugin.error(`更新失败: ${response.ErrorMessage || '未知错误'}`);
    }
  } catch (error: any) {
...
<style scoped>
/* TDesign Form 适配 */
.edit-room-modal :deep(.t-form__item) {
  margin-bottom: 16px;
}
