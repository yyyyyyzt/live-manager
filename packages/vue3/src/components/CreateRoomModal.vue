<template>
  <t-dialog
    ...
    @close="handleClose"
  >
    <t-form v-if="!showSuccess" class="create-room-form" :label-width="100" :colon="false">
      <!-- 直播间主播 -->
      <t-form-item label="直播间主播" :required-mark="true">
        <t-input
          v-model="formData.anchorId"
          placeholder="请输入主播ID"
          :status="anchorIdBytes > ANCHOR_ID_MAX_BYTES ? 'error' : 'default'"
          :tips="anchorIdBytes > ANCHOR_ID_MAX_BYTES ? `主播ID不能超过 ${ANCHOR_ID_MAX_BYTES} 字节` : ''"
        >
          <template #suffix>
      ...
      <!-- 直播间标题 -->
      <t-form-item label="直播间标题">
        <t-input
          v-model="formData.title"
          placeholder="请输入直播间标题"
          :status="titleBytes > 100 ? 'error' : 'default'"
          :tips="titleBytes > 100 ? '标题不能超过 100 字节' : ''"
        >
          <template #suffix>
      ...
      <!-- 布局模板 -->
      <t-form-item label="布局模板" :required-mark="true" :help="currentTemplateDesc">
        <t-select
          v-model="formData.seatTemplate"
      ...
      <!-- 最大麦位数 - 仅语聊房模板显示 -->
      <t-form-item v-if="formData.seatTemplate === 'AudioSalon' || formData.seatTemplate === 'Karaoke'" label="最大麦位数" help="语聊房模板可自定义麦位数量，受套餐包限制">
        <t-input-number
          :model-value="formData.maxSeatCount"
          @update:model-value="handleMaxSeatCountChange"
          :min="1"
          :max="16"
      ...
      <!-- OBS 推流选项 -->
      <t-form-item label="推流方式" help="勾选后创建成功将显示 OBS 推流信息">
        <t-checkbox v-model="useObsStreaming">
          使用 OBS 推流
        </t-checkbox>
      </t-form-item>
      ...
      <div class="custom-info-section">
        <div class="custom-info-toggle" @click="customInfoExpanded = !customInfoExpanded">
          <ChevronDownIcon v-if="customInfoExpanded" :size="16" />
          ...
          <div v-for="(info, index) in customInfos" :key="index" class="custom-info-row">
            <div class="custom-input-wrap">
              <t-input
                v-model="info.key"
                placeholder="请输入Key"
                :status="getKeyBytes(info.key) > CUSTOM_INFO_LIMITS.maxKeyBytes || isCustomInfoKeyMissing(info) ? 'error' : 'default'"
                :tips="getKeyBytes(info.key) > CUSTOM_INFO_LIMITS.maxKeyBytes ? `Key 超出 ${CUSTOM_INFO_LIMITS.maxKeyBytes} 字节限制` : isCustomInfoKeyMissing(info) ? '填写了 Value 时必须填写 Key' : ''"
              />
            </div>
            <div class="custom-input-wrap custom-value-wrap">
              <t-input
                ...
                :status="getValueBytes(info.value) > CUSTOM_INFO_LIMITS.maxValueBytes ? 'error' : 'default'"
                :tips="getValueBytes(info.value) > CUSTOM_INFO_LIMITS.maxValueBytes ? 'Value 超出 2KB 限制' : ''"
              />
            </div>
            <t-button variant="text" shape="circle" @click="removeCustomInfo(index)">
              <CloseIcon />
            </t-button>
          </div>
          <t-button style="width:80px;"
    ...
    <template #footer>
      <div class="dialog-footer">
        <template v-if="!showSuccess">
          <t-button variant="outline" shape="round" @click="handleClose">取消</t-button>
          <t-button theme="primary" shape="round" :loading="creating" :disabled="creating || !formData.anchorId.trim() || coverUploadRef?.isValidating || coverUploadRef?.hasUrlError" @click="handleSubmit">
            {{ creating ? '创建中...' : '创建' }}
          </t-button>
        </template>
        <template v-else>
          <t-button theme="primary" shape="round" @click="handleComplete">
            完成
          </t-button>
        </template>
      </div>
...
import { ChevronDownIcon, ChevronRightIcon, AddIcon, CloseIcon, CopyIcon } from 'tdesign-icons-vue-next';
...
const handleCopy = async (text: string, label: string) => {
  try {
    ...
    MessagePlugin.error(`复制失败: ${error.ErrorInfo || error.message || '未知错误'}`);
  }
};

const addCustomInfo = () => {
  if (customInfos.value.length >= CUSTOM_INFO_LIMITS.maxCount) {
    MessagePlugin.error(`自定义信息最多 ${CUSTOM_INFO_LIMITS.maxCount} 条`);
    return;
  }
...
const handleSubmit = async () => {
  if (!formData.value.anchorId.trim()) {
    MessagePlugin.error('请输入直播间主播ID');
    return;
  }

  if (anchorIdBytes.value > ANCHOR_ID_MAX_BYTES) {
    MessagePlugin.error(`主播ID不能超过 ${ANCHOR_ID_MAX_BYTES} 字节（当前 ${anchorIdBytes.value} 字节）`);
    return;
  }

  if (titleBytes.value > 100) {
    MessagePlugin.error(`标题不能超过 100 字节（当前 ${titleBytes.value} 字节）`);
    return;
  }

  if (customInfos.value.some(isCustomInfoKeyMissing)) {
    MessagePlugin.error('自定义信息填写了 Value 时必须填写 Key');
    return;
  }
  ...
  try {
    ...
    try {
      ...
      MessagePlugin.error(err instanceof ImageUploadResolveError ? err.message : `封面处理失败: ${err.message || '未知错误'}`);
    ...
    if (response.ErrorCode === 0 || response.ErrorCode === undefined) {
      ...
      MessagePlugin.success(useObsStreaming.value ? '直播间创建成功，OBS 已配置' : '直播间创建成功');
    } else {
    ...
    MessagePlugin.error(`请求失败: ${error.message || '网络错误'}`);
  } finally {
    creating.value = false;
  }
};
...
<style scoped>
/* TDesign Form 适配 */
.create-room-modal :deep(.t-form__item) {
  margin-bottom: 16px;
}

.create-room-modal :deep(.t-form__item:last-child) {
  margin-bottom: 0;
}
...
.create-room-modal :deep(.t-form__controls-content .image-upload-container) {
  width: 100%;
}
