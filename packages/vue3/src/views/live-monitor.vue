<template>
  <div class="live-monitor-page">
    ...
    <t-dialog
      ...
      header="确认要强制关播吗？"
      :confirm-btn="{ content: closeConfirm.closing ? '关播中...' : '确认', loading: closeConfirm.closing, theme: 'primary', shape: 'round' }"
      :cancel-btn="{ content: '取消', disabled: closeConfirm.closing, shape: 'round' }"
...
import { StopCircleIcon, CloseIcon, SoundIcon, SoundMuteIcon, FullscreenIcon, SearchIcon, RefreshIcon } from 'tdesign-icons-vue-next';
...
const handleSearch = async (keyword?: string) => {
  ...
  try {
    ...
    if (response.ErrorCode !== 0) {
      MessagePlugin.error(getErrorMessage(response.ErrorCode, response.ErrorInfo, `没有搜到「${input}」`));
    ...
    if (roomInfo) {
      ...
      MessagePlugin.error(`没有搜到「${input}」`);
      await loadAndPlayNewPage(1, searchVersion);
    }
  } catch (error: any) {
...
const handleRefresh = async () => {
  ...
  try {
    ...
    MessagePlugin.error(`刷新失败${errorInfo ? ` (${errorInfo})` : ''}`);
  } finally {
    if (!isUnmounted.value) {
